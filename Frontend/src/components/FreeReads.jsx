...
function FreeBook() {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/freereads`);
        setBooks(res.data);
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/freereads/upload`, formData, {
          ...
          {books.length > 0 ? (
            books.map((book) => (
              ...
              >
                ...
                <a
                  href={`${process.env.REACT_APP_API_URL}/freereads/file/${book.gridfsId}`}
                  target="_blank"
