...
function Cards({ item, onRemove }) {
  ...
  useEffect(() => {
    const fetchWishlistStatus = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:4001/wishlist/${user._id}`);
  ...
  const toggleWishlist = async () => {
    ...
    try {
      if (isWishlisted) {
        const response = await axios.delete("http://localhost:4001/wishlist/remove", {
        ...
        const response = await axios.post("http://localhost:4001/wishlist/add", {
          userId: user._id,
          book: item,
        });
